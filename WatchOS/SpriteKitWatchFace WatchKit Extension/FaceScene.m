...
- (instancetype)initWithCoder:(NSCoder *)coder
{
	...
	if (self) {
		...
		NSLog(@"Permutations per theme = %lu", FaceStyleMAX*NumeralStyleMAX*TickmarkStyleMAX*(TickmarkShapeMAX*2)*ColorRegionStyleMAX*CenterDiscStyleMAX);
		NSLog(@"Total permutations = %lu", ThemeMAX*FaceStyleMAX*NumeralStyleMAX*TickmarkStyleMAX*(TickmarkShapeMAX*2)*ColorRegionStyleMAX*CenterDiscStyleMAX);
...
-(void)setupTickmarksForRoundFaceWithLayerName:(NSString *)layerName
{
	...
	for (int i = 0; i < 48; i++)
	{
		...
		{
			if ((self.tickmarkStyle == TickmarkStyleStackMajor) || (self.tickmarkStyle == TickmarkStyleMinor) || i % 400 != 0)
			{
	...
	for (int i = 0; i < 24; i++)
	{
		...
		*/
        if (i % 2 != 0) { // Only add every other number, ()
            ...
            NSAttributedString *labelText = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%i", i == 0 ? 24 : i] attributes:attribs];
            
            SKLabelNode *numberLabel = [SKLabelNode labelNodeWithAttributedText:labelText];
            numberLabel.position = CGPointMake((workingRadius-labelMargin) * -sin(angle - (9*2*M_PI)/24.0), (workingRadius-labelMargin) * cos(angle - (9*2*M_PI)/24.0) - 7);
...
-(void)setupColors
{
	...
	switch (self.theme) {
		...
		case ThemeTidepod:
		{
			colorRegionColor = [SKColor colorWithRed:0.150 green:0.150 blue:0.150 alpha:1.000];
			faceBackgroundColor = [SKColor colorWithRed:0.000 green:0.000 blue:0.000 alpha:1.000];
			inlayColor = [SKColor colorWithRed:0.953 green:0.569 blue:0.196 alpha:1.000];
			majorMarkColor = [SKColor colorWithRed:0.800 green:0.800 blue:0.800 alpha:1.000];
			minorMarkColor = majorMarkColor;
			handColor = [SKColor colorWithRed:0.800 green:0.800 blue:0.800 alpha:1.000];
			textColor = [SKColor colorWithRed:0.800 green:0.800 blue:0.800 alpha:1.000];
			secondHandColor = majorMarkColor;
			break;
		}
		case ThemeBretonnia:
...
-(void)updateHands
{
	...
	hourHand.zRotation =  - (2*M_PI)/24.0 * (CGFloat)(components.hour%24 + 1.0/60.0*components.minute) - (9*2*M_PI)/24.0;
